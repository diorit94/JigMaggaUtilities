...
module.exports = {
    ...
    getConfigPaths: function (basePath, domain, page, callback) {
        ...
        async.waterfall([
            function(cb){
                cb(null,_.zip([
                    getPossibleConfigsForDomain(defaultDomainName, page),
                    getPossibleConfigsForDomain(domain, page)
                ]));
            },
            function(zippedArrays, cb){
                cb(null,_.flatten(zippedArrays));
            },
            function(flattenArrays, cb){
                flattenArrays.unshift(path.join(basePath, defaultPageConfigName));
                cb(null, flattenArrays)
            }
        ], function (err, result) {
            callback(null, result);
        });
